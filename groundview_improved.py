...
SQSIZE = 10800
LONMIN = -116.0005555556
LATMAX = 37.0005555556
INITIAL_SQUARES = 100
ROUGHNESS_THRESHOLD = 21
...
def InitGL(Width, Height):				# We call this right after our OpenGL window is created.
	...
	geo = gdal.Open('imgn37w116_13.img');
	arr = geo.ReadAsArray();
	dim = arr.shape;
	...
	for x in range(len(pointsList)):
		...
		vectordict[pointsList[x]] = numpy.array([t[0],t[1],t[2]])
	...
	for x in range(len(rectList)):
		...
		while ((i<len(left)) and (j<len(upper))):
			...
			xupper = upper[j][0]
			yupper = upper[j][1]
			edge1 = ((xleft,yleft),(xupper,yupper))
			edge2 = ((xupper,yupper),(xa,ya))
			edge3 = ((xa,ya),(xleft,yleft))
			edgedict[edge1] = (xa,ya)
			edgedict[edge2] = (xleft,yleft)
			edgedict[edge3] = (xupper,yupper)
		...
		while (i>=0) and (j>=0):
			...
			xlower = lower[j][0]
			ylower = lower[j][1]
			edge1 = ((xa,ya),(xright,yright))
			edge2 = ((xright,yright),(xlower,ylower))
			edge3 = ((xlower,ylower),(xa,ya))
			edgedict[edge1] = (xlower,ylower)
			edgedict[edge2] = (xa,ya)
			edgedict[edge3] = (xright,yright)
	...
	edges_to_check = set(edgedict.keys())
	flipped = 0
	while edges_to_check:
		x = edges_to_check.pop()
		p1 = x[0]
		p2 = x[1]
		q1 = edgedict.get((p2,p1))
		q2 = edgedict.get((p1,p2))
		if q1 != None:
			if intersect([q1,q2],x):
				d1 = numpy.dot(vectordict[p1]-vectordict[p2],vectordict[p1]-vectordict[p2])
				d2 = numpy.dot(vectordict[q1]-vectordict[q2],vectordict[q1]-vectordict[q2])
				if (d2 < d1):
					flipped = flipped+1
					del edgedict[(p1,p2)]
					del edgedict[(p2,p1)]
					...
					edgedict[(q1,p2)] = q2
					edgedict[(p2,q2)] = q1
					edges_to_check.add((q2,p1))
					edges_to_check.add((p1,q1))
					edges_to_check.add((q1,p2))
					edges_to_check.add((p2,q2))

	print('number flipped',flipped)
	
	edgeslist = set(edgedict.keys())
	triangleList = []
	while edgeslist:
		x = edgeslist.pop()
		p3 = edgedict[x]
		vectora = transformarray[pointsdict[x[0]]][0:3]
		vectorright = transformarray[pointsdict[x[1]]][0:3]
		vectorleft = transformarray[pointsdict[p3]][0:3]
		...
		normalsdict[x[0]] = normalsdict[x[0]]+cross
		normalsdict[x[1]] = normalsdict[x[1]]+cross
		normalsdict[p3] = normalsdict[p3]+cross
		triangleList.extend([pointsdict[x[0]],pointsdict[x[1]],pointsdict[p3]])
		edgeslist.remove((x[1],p3))
		edgeslist.remove((p3,x[0]))
...
def transform(x1, y1, h):
	lon = math.radians(LONMIN+float(y1)/SQSIZE);
	lat = math.radians(LATMAX-float(x1)/SQSIZE);
	n = a/numpy.sqrt(1-e2*(math.sin(lat))**2)
...
def intersect(e1, e2):
	...
	p3 = numpy.array(e2[0])
	p4 = numpy.array(e2[1])
	c1 = getcross(p2-p1, p3-p1)
	c2 = getcross(p2-p1, p4-p1)
	c3 = getcross(p4-p3, p1-p3)
	c4 = getcross(p4-p3, p2-p3)
